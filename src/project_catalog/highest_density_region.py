...
def compute_1d_hdrs(galactic_binary: GalacticBinary, candidate_index: int, grid_size=1000):
    ...
    for param in galactic_binary.binary_params:
        ...
        samples = np.asarray(galactic_binary.get_binary_parameters_chain()[param])
        kde = gaussian_kde(samples)
        x_grid = np.linspace(samples.min(), samples.max(), grid_size)
        pdf = kde.evaluate(x_grid)
        dx = x_grid[1] - x_grid[0]
        # Interpolate PDF at x_true
        p_true = kde.evaluate([galactic_binary.get_binary_parameters_candidates()[param].iloc[candidate_index]])[0]
...
def compute_sky_location_hdr(galactic_binary: GalacticBinary, candidate_index: int):
    """
    ...
    X = galactic_binary.get_sky_location_chain().values.T
    ...
    tv = np.asarray(galactic_binary.get_sky_location_candidates().iloc[candidate_index]).reshape(-1, 1)
...
def compute_8d_hdr(galactic_binary: GalacticBinary, candidate_index: int):
    ...
    """
    # 1) prepare the data for gaussian_kde: shape (D, N)
    X = galactic_binary.get_binary_parameters_chain().values.T
    kde = KernelDensity(kernel='gaussian').fit(X.T)
    # kde = gaussian_kde(X, bw_method=bw_method)
    ...
    # 3) density at the true point
    #    must be shape (D, M); here M=1
    tv = np.asarray(galactic_binary.get_binary_parameters_candidates().iloc[candidate_index]).reshape(-1,1)
    p_true = kde.score_samples(tv.T)[0]
